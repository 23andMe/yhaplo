...
class SNP(object):
    ...
    def __init__(self, name, haplogroup, position, ancestral, derived):
        if SNP.tree is None:
            sys.exit(
                "ERROR. Before instantiating, must call SNP.setClassVariables(tree)."
            )
    ...
    def setLabel(self, label):
        ...
        self.label = label
        self.labelLettersRank, self.labelLetters, self.labelNumber = SNP.parseLabel(
            label, SNP.config.snpLabelLettersRankDict
        )
    ...
    def addName(self, name):
        ...
        if SNP.isApreferredName(self.label):
            ...
            labelLettersRank, labelLetters, labelNumber = SNP.parseLabel(
                name, SNP.config.snpLabelLettersRankDict
            )
    ...
    def isApreferredName(name):
        ...
        return (
            name in SNP.tree.preferredSNPnameSet
    ...
    def prioritySortMarkerList(markerList):
        ...
        markerList = sorted(
            markerList, key=attrgetter("isRepresentative"), reverse=True
        )
...
class PlatformSNP(object):
    ...
    def getConsensusGenotypeFromAblock(self, ablock):
        ...
        genotypeList = [
            PlatformSNP.getGenotypeFromAblock(ablock, ablockIndex)
...
class DroppedMarker(object):
    ...
    def setSortVariables(self):
        ...
        self.labelLettersRank, self.labelLetters, self.labelNumber = SNP.parseLabel(
            self.name, SNP.config.snpLabelLettersRankDict
        )
