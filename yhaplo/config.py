...
class Config(object):
    ...
    snpLabelLettersRankString = (
        "M P V L CTS AF B F Page U PF Z SK"  # for prioritization
    )
    superfluousSNPtextList = ["IMS-", "-null"]  # stripped out of snp names
    multiCharHgTruncString = (
    ...
    snpLabelLettersRankDict = {
        letters: rank for rank, letters in enumerate(snpLabelLettersRankString.split())
    }
    ...
    snpMetaColList = [
        "platform:chrom",
        "platform:pos",
    ]  # features to draw from SNP metadata
    ...
    customerIDcol = "resid"
    customerPlatformColList = [
        "is_v%d" % platformVersion
    ...
    customerMetaColList = [
        customerIDcol
    ] + customerPlatformColList  # for whenever metadata used
    CustomerTuple = namedtuple(
        "CustomerTuple", customerMetaColList + [customerPrevHaplogroupCol]
    )
    ...
    isoggCorrectionsFNlist = [
        "%s/isogg.correct.coordinate.txt" % inDir,
        "%s/isogg.correct.polarize.txt" % inDir,
    ]
    isoggOmitFNlist = [
        "%s/isogg.omit.bad.txt" % inDir,
    ...
    thousandYhgFN = os.path.join(
        softwareDir, "1000Y/haplogroups/4.haplogroups.called.txt"
    )
    ...
    def setParamsGeneral(self, outDir):
        ...
        if self.args.all_aux_output:
            aux_output_arg_list = [
                "writeAncDerCounts",
                "writeHaplogroupPaths",
                "writeHaplogroupPathsDetail",
                "writeDerSNPs",
                "writeDerSNPsDetail",
                "writeAncSNPs",
                "writeAncSNPsDetail",
        ...
        self.test1000Y = (
            self.args.test1000Yall
            or self.args.test1000YplatformVersion
            or self.args.test1000Ysubset
            or self.args.test1000YoneID
        )
        self.test1000YformatSpecified = (
            self.args.test1000Yvcf or self.args.test1000Yvcf4
        )
    ...
    def validateParams(self):
        ...
        if self.test1000Y:
            if self.args.dataFN:
                sys.exit(
                    "ERROR. Do not specify a 1000Y test option "
            ...
            sys.exit(
                "ERROR. Only specify a 1000Y data format "
        ...
        if self.args.ablockDSname or self.args.ablocks_dir:
            if not self.args.dataFN or not self.args.dataFN.endswith(".resid.txt"):
                sys.exit(
                    "ERROR. Provide a resid list (-i file_label.resid.txt) "
                    "when specifying\n    a non-default ablock dataset "
                    "name or ablock directory"
                )
            if self.args.compareToMetadata:
                sys.exit(
                    "ERROR. Will not check metadata when using "
    ...
    def setOutputFileNamesAndOpenSome(self):
        ...
        for fn, tp in six.iteritems(
            Config.phyloOutputFNtpDictDict["withOutdirAndIsoggDate"]
        ):
        ...
        if self.args.writeHaplogroupPaths or self.args.writeHaplogroupPathsDetail:
            self.haplogroupPathsFN = self.constructOutFileName(
                Config.haplogroupPathsFNtp
            )
        ...
        if self.args.writeHaplogroupsRealTime:
            self.haplogroupRealTimeFN = self.constructOutFileName(
                Config.haplogroupRealTimeFNtp
            )
            self.haplogroupRealTimeFile = open(self.haplogroupRealTimeFN, "w", 1)

        if self.args.haplogroupToListGenotypesFor:
            self.hgGenosFN = (
                self.constructOutFileName(Config.hgGenosFNtp)
    ...
    def get23andMeDatasets(self):
        ...
        try:
            ...
            from rtk23.dataset import UnknownDatasetException, dataset_factory
            from rtk23.lib.coregen import VALUE_TO_CALL as ablockCodeToGenotypeDict
        except ImportError:
            sys.exit(
                "ERROR. Cannot import from rtk23.\n"
    ...
    def setPos2ablockIndexListDict(self):
        ...
        chrom_pos_tuple_list = zip(
            *[snpMetaArrayDict[column] for column in Config.snpMetaColList]
        )
    ...
    def setCommandLineArgs(self):
        ...
        group.add_argument(
            "-o", "--outDir", dest="outDir", metavar="dirName", help="output directory"
        )

        group = parser.add_argument_group("run on example data")
        group.add_argument(
            "-ex",
            "--example_1000Y_subset",
            action="store_true",
        ...
        group.add_argument(
            "-hpd",
            "--haplogroupPathsDetail",
            dest="writeHaplogroupPathsDetail",
            action="store_true",
            help="sequence of branch labels from root to call,\n"
        ...
        group.add_argument(
            "-hg",
            "--hgGenos",
            dest="haplogroupToListGenotypesFor",
            metavar="haplogroup",
        ...
        group.add_argument(
            "-pt",
            "--platformTrees",
            dest="writePlatformTrees",
            action="store_true",
            help="23andMe: write trees whose branch lengths are numbers\n"
            "of platform sites",
        )

        group = parser.add_argument_group("compare to previously called haplogroups")
        group.add_argument(
            "-mdh",
            "--compareToMetadata",
            dest="compareToMetadata",
            action="store_true",
        ...
        group.add_argument(
            "-po", "--primaryOnly", action="store_true", help="do NOT import ISOGG SNPs"
        )
        group.add_argument(
            "-r",
            "--root",
            dest="alternativeRoot",
            metavar="haplogroup",
        ...
        group.add_argument(
            "-tv",
            "--test1000YplatformSites",
            type=int,
            dest="test1000YplatformVersion",
            metavar="version",
        ...
        group.add_argument(
            "-v", "--version", action="version", version="yhaplo %s" % __version__
        )
