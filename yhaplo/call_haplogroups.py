...
def call_haplogroups(
    useDefaultCmdLineArgs=False, suppressOutputAndLog=False, outDir=None, residList=None
):
    "configures run, builds tree, and calls haplogroups"

    config = Config(
        useDefaultCmdLineArgs=useDefaultCmdLineArgs,
...
def call_haplogroups_for_resid_list(resid_list):
    ...
    call_haplogroups(
        useDefaultCmdLineArgs=True, suppressOutputAndLog=True, residList=resid_list
    )
