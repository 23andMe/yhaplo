...
py_version_start=$(pyenv global)

rm -fr output
for py_version in 2.7.18 3.8.6; do
    echo -e "Python ${py_version}\n\n"
    pyenv global ${py_version}
    
    echo -e "Text Input\n"
    yhaplo --example_1000Y_subset --hgGenos Q
    
    echo -e "\n\nSingle-Sample VCF Input\n"
    yhaplo --example_single_sample_vcf --hgGenos Q
    
    echo -e "\n\nMulti-Sample VCF Input\n"
    if [ $# == 1 ] && [ "$1" == "m" ]; then
        if [ -e ${multi_sample_vcf_fn} ]; then
            yhaplo -i ${multi_sample_vcf_fn} --hgGenos Q
        else
    ...
    echo -e "\n\nTree Plotter\n"
    plot_tree -n ${nwk_fn} | tee ${tree_drawing_fn}
    
    echo
    py_version_major=$(echo $py_version | cut -f1 -d".")
    out_dir=output.yhaplo.py${py_version_major}
    rm -fr ${out_dir}
    mv output ${out_dir}
done;
pyenv global ${py_version_start}


echo -e "\n\n\n----------------------------------------------------------------------"
echo -e "Expected versus py2\n"
if [ -d ${expected_output_dir} ]; then
    ls ${expected_output_dir} \
    | xargs -I {} echo diff ${expected_output_dir}/{} ${py2_output_dir}/{} \
    | sh -v
else
...
ls ${py2_output_dir} \
| xargs -I {} echo diff ${py2_output_dir}/{} ${py3_output_dir}/{} \
| sh -v
